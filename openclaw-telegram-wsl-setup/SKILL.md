...
11. Install or verify the local OpenClaw Control Center on Windows when this skill bundle includes `tools/openclaw-local-monitor`.
   ...
   - Token/cost cards should use the optional offline usage cache at `~/.openclaw/monitor-cache/usage-summary.json`. Install `Install-UsageCache.ps1` when the user wants these cards. The collector scans local session files on a WSL timer, about every 10 minutes; it must not connect to gateway, restart OpenClaw, change config, or touch secrets. Token cards show today's flow; the cost card shows the current natural month's accumulated local estimate and resets at the next month boundary.
   - Recent silent-failure explanations should use the optional reliability observer cache at `~/.openclaw/monitor-cache/reliability-status.json`. Install `Install-ReliabilityObserver.ps1` when the user wants the panel to explain model overload, Telegram delivery failure, provider/network timeout, session lock, context overflow, or gateway lifecycle incidents. The observer reads only local logs/journal/stability files; it must not send Telegram messages, auto-retry commands, connect to gateway, call `tasks audit/show`, restart OpenClaw, change config, or touch secrets.
...
- Token/cost shown in the panel must come from the offline cache `~/.openclaw/monitor-cache/usage-summary.json`, generated by the optional `openclaw-usage-cache` WSL timer about every 10 minutes. The main panel must not scan monthly sessions or call gateway RPCs to fill token/cost cards. Token cards represent today's local flow; the cost card represents the current natural month's accumulated local estimate and resets at the next month boundary. Treat amounts as local estimates, not provider invoices.
- Reliability notices shown in the panel should come from the optional cache `~/.openclaw/monitor-cache/reliability-status.json`, generated by `openclaw-reliability-observer` about every minute. This cache is for explanation only: no Telegram send, no auto retry, no gateway restart, no `tasks audit/show`, and no config/model/binding/session changes.
