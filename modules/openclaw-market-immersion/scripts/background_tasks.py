...
VALID_STATUSES = {"pending", "running", "succeeded", "failed", "needs_review", "cancelled"}
...
def finish_task(task_id: str, *, artifacts: list[str | Path] | None = None, summary: str = "") -> dict[str, Any]:
    if artifacts:
        add_artifacts(task_id, artifacts)
    return update_task(task_id, status="succeeded", completed_at=now_iso(), error_kind="", error_summary="", result_summary=summary, event="succeeded")
