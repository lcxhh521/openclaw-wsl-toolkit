...
7. 如果用户在配置里启用 Telegram，正式阶段只推送 Notion 链接；不要把本地 Markdown、manifest 或输出目录当成用户提醒内容。
...
原始消息流是审计材料，应尽量忠实保留来源字段：没有标题就显示序号和正文，不用正文首句生造标题；来源时间统一格式化，接口内部时间戳不能原样露出；多源重复行情可按主题、正文相似度和近似数字合并为“来源等 N 源”。
