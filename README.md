...
仓库不会内置具体的人民日报/政策文本解读 prompt。安装市场信息浸泡模块后，用户需要在本机创建自己的私有 prompt 文件，并在模块配置里填写路径，例如：
...
私有 prompt 文件不要提交到 GitHub。完整操作步骤见 `modules/openclaw-market-immersion/README.md` 的「配置自己的人民日报深读 prompt」。
