...
- What cost has OpenClaw recorded locally from this calendar month's `usage.cost`, grouped by model?
...
- During cold startup, keep the panel probe light until the startup progress reaches ready: run only gateway probe and Telegram channel status first, then defer task, TaskFlow, audit, logs, token, cost, session, and workspace-artifact reads. The goal is to show reliable startup progress without adding extra load or WebSocket churn while OpenClaw is still starting channels and sidecars.
...
- The panel should automatically update display data on its timer. A manual button should not be a meaningless duplicate refresh; label it `重新检测` and make it actively wake WSL, lightly start or nudge `openclaw-gateway.service`, and reread WSL/OpenClaw status. This manual path must not stop OpenClaw, reset tasks, touch tokens, or change configuration. Automatic timer refreshes should remain read-only and should not secretly start or stop OpenClaw.
- Add an in-window bounded hover hint to `重新检测` explaining that it wakes WSL, lightly starts/checks the gateway, and rereads current status without changing config, resetting tasks, or touching tokens. Do not use a native tooltip that can overflow outside the app window.
